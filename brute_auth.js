      ...
      if(true){
        if(test[i].length>2){
          setTimeout(function(){
            try{
              console.log('i: ' + i)
              console.log("test: d" + test[i]);
              console.log("length: " + (parseInt(test[i].length)+1));
              client.write(Buffer.from("00"+(test[i].length+1 + password.length + 8).toString(16)+"0003"+
              "000" + ((parseInt(test[i].length)+1)).toString(16) + Buffer.from("d"+test[i],'ascii').toString('hex') +
...
client.on('data', function(data) {
  ...
  if(data.toString('hex',2,4) == Buffer.from('ff20','hex').toString('hex')){
    ...
    var i;
    //var location = data.indexOf('0003','hex');
    for (i = 12; i < message_length;) {
      if(data.toString('hex',i,i+1) == Buffer.from('04','hex').toString('hex')){
        console.log("[END OF TRANSMISSION] ASCII code, searching for new beginning")
        i = data.indexOf('0003',i+1,'hex');
      }
      if(data.toString('hex',i,i+1) == Buffer.from('03','hex').toString('hex')){
        console.log("[END OF TEXT] ASCII code, searching for new beginning")
        i = data.indexOf('0003',i+1,'hex');
      }
      //console.log(location);
      //console.log('name_length: ' + data.toString('hex',i,i+2));
      //var num_stock = parseInt(data.readIntBE(i,2));
      //console.log("number of stock: " + num_stock);
      //console.log(data.toString('hex',i,i+4));
      //name_length = parseInt(data.toString('hex',i,i+2),16);
      name_length = 3;
      console.log("entry number: " + i);
      //console.log("name length: " + name_length);
      console.log(data.toString('utf8',i+2,i+2+name_length));
      //console.log(data.toString('hex',i+2,i+2+name_length));
      i = i + 2 + name_length;
    }
  }
